...
Digest.send = async function (data) {
	...
	await batch.processArray(data.subscribers, async (uids) => {
		...
		await Promise.all(userData.map(async (userObj, index) => {
			...
			if (!unreadNotifs.length && !topics.top.length && !topics.popular.length &&
...
async function getTermTopics(term, uid) {
	...
	const popular = data.topics.filter(t => t.postcount > 1)
		.sort((a, b) => b.postcount - a.postcount)
		.slice(0, 10);
	const popularTids = popular.map(t => t.tid);
	const top = data.topics.filter(t => t.votes > 0 && !popularTids.includes(t.tid))
		.sort((a, b) => b.votes - a.votes)
		.slice(0, 10);
	const topTids = top.map(t => t.tid);
	const recent = data.topics.filter(t => !topTids.includes(t.tid) && !popularTids.includes(t.tid))
