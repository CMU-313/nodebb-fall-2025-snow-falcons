...
module.exports = function (module) {
	module.sortedSetIntersectCard = async function (keys) {
		...
		for (let i = 0; i < otherSets.length; i++) {
			const query = {
				_key: otherSets[i],
				value: {
					$in: items.map(i => i.value),
				},
			};
			if (i === otherSets.length - 1) {
				// Sequential processing required for intersection
				// eslint-disable-next-line no-await-in-loop
				return await objects.countDocuments(query);
			}
			// Sequential processing required for intersection
			// eslint-disable-next-line no-await-in-loop
			items = await objects.find(query, {
				projection: {
					_id: 0,
					value: 1,
				},
	...
	async function countSets(sets, limit) {
		const objects = module.client.collection('objects');
		const counts = await Promise.all(sets.map(s => objects.countDocuments({
			_key: s,
		}, {
			limit: limit || 25000,
		})));
	...
	async function intersectSingle(params) {
		...
		for (let i = 0; i < otherSets.length; i++) {
			...
			// Sequential processing required for cursor iteration
			// eslint-disable-next-line no-await-in-loop
	...
	async function intersectBatch(params) {
		...
		while (!done) {
			...
			// Sequential processing required for intersection batch
			// eslint-disable-next-line no-await-in-loop
