...
SELECT z."value", z."score"
 ...
 ORDER BY z."score" ${sort}, z."value" ${sort}`, [setKey, min, max]));
		...
		let iteration = 1;
		while (true) {
			// Sequential processing required for cursor batch iteration
			...
			try {
				if (iteration > 1 && options.interval) {
					// Sequential processing required for interval delay
					// eslint-disable-next-line no-await-in-loop
					await sleep(options.interval);
				}
				// Sequential processing required for batch processing
				// eslint-disable-next-line no-await-in-loop
				await process(rows);
				iteration += 1;
			} catch (err) {
				// Sequential processing required for error handling
				// eslint-disable-next-line no-await-in-loop
