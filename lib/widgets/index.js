...
widgets.checkVisibility = async function (data, uid) {
	...
	const isExpired = (data.startDate && Date.now() < new Date(data.startDate).getTime()) ||
...
widgets.saveLocationsOnThemeReset = async function () {
	...
	results.forEach(({ area, widgetsAtLocation }) => {
		if (widgetsAtLocation.length) {
...
widgets.moveMissingAreasToDrafts = async function () {
	...
	try {
		...
		const processPromises = locationEntries.map(async ([template, tplLocations]) => {
			const locationPromises = tplLocations.map(async (location) => {
				...
				if (!locationExists) {
					...
					return widgetsAtLocation;
				}
				return [];
			});
			const widgetsArrays = await Promise.all(locationPromises);
			return widgetsArrays.flat();
		});
		const widgetsArrays = await Promise.all(processPromises);
		saveDraftWidgets = saveDraftWidgets.concat(widgetsArrays.flat());
		await widgets.setArea({
			template: 'global',
			location: 'drafts',
...
widgets.reset = async function () {
	...
	const areaDataArrays = await Promise.all(areaPromises);
	saveDrafts = saveDrafts.concat(areaDataArrays.flat());
	await widgets.setArea({
		template: 'global',
		location: 'drafts',
		widgets: saveDrafts,
