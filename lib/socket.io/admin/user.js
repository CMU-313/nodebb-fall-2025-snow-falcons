...
User.makeAdmins = async function (socket, uids) {
	...
	const makeAdminPromises = uids.map(async (uid) => {
...
User.removeAdmins = async function (socket, uids) {
	...
	const removeAdminPromises = uids.map(async (uid) => {
	...
	await Promise.all(removeAdminPromises);
};
...
User.validateEmail = async function (socket, uids) {
	...
	const validatePromises = uids.map(async (uid) => {
