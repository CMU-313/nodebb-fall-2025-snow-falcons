...
UserRoles.assignRole = async function (socket, {
  uid,
  role
}) {
  ...
  const [isTargetAdmin, isTargetGlobalMod] = await Promise.all([privileges.users.isAdministrator(uid), privileges.users.isGlobalModerator(uid)]);
...
UserRoles.createRole = async function (socket, {
  roleName
}) {
  await requireAdmin(socket);
  ensureTaggingEnabled();
...
UserRoles.deleteRole = async function (socket, {
  roleName
}) {
  await requireAdmin(socket);
  ensureTaggingEnabled();
