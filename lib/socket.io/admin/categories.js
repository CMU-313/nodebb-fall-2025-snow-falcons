...
Categories.copyPrivilegesToChildren = async function (socket, data) {
	...
	const copyPromises = children.map(child => copyPrivilegesToChildrenRecursive(
		data.cid,
		child,
		data.group,
		data.filter,
...
async function copyPrivilegesToChildrenRecursive(parentCid, category, group, filter) {
	...
	const childPromises = category.children.map(child => copyPrivilegesToChildrenRecursive(
		parentCid,
		child,
		group,
		filter,
