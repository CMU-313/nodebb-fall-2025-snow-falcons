...
Actors.assertGroup = async (ids, options = {}) => {
  ...
  await Promise.all([db.setObjectBulk(bulkSet), db.sortedSetAdd('usersRemote:lastCrawled', groups.map(() => now), groups.map(p => p.id)), db.sortedSetAddBulk(queries.searchAdd), db.setObject('handle:cid', queries.handleAdd), _migratePersonToGroup(categoryObjs)]);
  return categoryObjs;
};
async function _migratePersonToGroup(categoryObjs) {
  ...
  await Promise.all(ids.map(async id => {
    ...
    await db.sortedSetAdd(`cid:${id}:uid:watch:state`, followers.map(() => categories.watchStates.tracking), followers.map(({
      value
    }) => value));
    await user.deleteAccount(id);
  }));
