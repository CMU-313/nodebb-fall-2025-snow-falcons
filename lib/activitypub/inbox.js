...
inbox.update = async req => {
  ...
  switch (true) {
    ...
    case object.type === 'Tombstone':
      {
        const [isNote, isMessage /* , isActor */] = await Promise.all([posts.exists(object.id), messaging.messageExists(object.id)
...
inbox.delete = async req => {
  ...
  try {
    if (!type) {
      ({
        type
      } = await activitypub.get('uid', 0, pid));
    }
    if (type === 'Tombstone') {
      method = 'delete';
    }
  ...
  const [isNote /* , isActor */] = await Promise.all([posts.exists(pid)
  // db.isSortedSetMember('usersRemote:lastCrawled', object.id),
  ]);
  switch (true) {
    case isNote:
...
inbox.accept = async req => {
  ...
  if (type === 'Follow') {
    if (localType === 'user') {
      ...
      await Promise.all([db.sortedSetRemove(`followRequests:uid.${id}`, actor), db.sortedSetAdd(`followingRemote:${id}`, timestamp, actor), db.sortedSetAdd(`followersRemote:${actor}`, timestamp, id) // for followers backreference and notes assertion checking
      ]);
      ...
      await Promise.all([db.sortedSetRemove(`followRequests:cid.${id}`, actor), db.sortedSetAdd(`cid:${id}:following`, timestamp, actor), db.sortedSetAdd(`followersRemote:${actor}`, timestamp, `cid|${id}`) // for notes assertion checking
      ]);
    }
  }
};
