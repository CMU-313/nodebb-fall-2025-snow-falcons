...
Notes.assert = async (uid, input, options = {
	...
	const hasRelation = uid || hasTid || options.skipChecks || options.cid ||
	...
	for (const post of unprocessed) {
		...
		try {
			// Sequential processing required for topic reply
			// eslint-disable-next-line no-await-in-loop
			await topics.reply(post);
			// Sequential processing required for recipient updates
			// eslint-disable-next-line no-await-in-loop
