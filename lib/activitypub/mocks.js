...
Mocks._normalize = async object => {
  ...
  switch (true) {
    // non-string attributedTo handling
...
Mocks.post = async objects => {
  ...
  const posts = await Promise.all(objects.map(async object => {
    ...
    if (!activitypub._constants.acceptedPostTypes.includes(object.type) || !activitypub.helpers.isUri(object.id) // sanity-check the id
    ) {
      return null;
    }
    let {
...
Mocks.actors.user = async uid => {
  ...
  return {
    ...
    // displayname is escaped, fullname is not
...
Mocks.actors.category = async cid => {
  let {
    ...
    backgroundImage
  } = await categories.getCategoryFields(cid, ['name', 'handle', 'slug', 'description', 'descriptionParsed', 'federatedDescription', 'backgroundImage']);
...
Mocks.notes.public = async post => {
   ...
   */
  let audience = utils.isNumber(post.category.cid) ?
  // default
