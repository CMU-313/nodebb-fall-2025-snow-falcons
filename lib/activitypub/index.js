...
ActivityPub.send = async (type, id, targets, payload) => {
  ...
  batch.processArray(inboxes, async inboxBatch => Promise.all(inboxBatch.map(async uri => sendMessage(uri, id, type, payload))), {
    batch: 50,
    interval: 100
  });
};
ActivityPub.record = async ({
  ...
  await Promise.all([db.sortedSetAdd(`activities:datetime`, now, id), db.sortedSetAdd('domains:lastSeen', now, hostname), analytics.increment(['activities', `activities:byType:${type}`, `activities:byHost:${hostname}`])]);
};
ActivityPub.buildRecipients = async function (object, {
  ...
  if (pid) {
    ...
    const participants = (await db.getSortedSetMembers(`tid:${tid}:posters`)).filter(uid => !utils.isNumber(uid)); // remote users only
...
ActivityPub.probe = async ({
  ...
  // Known resources
  const [isNote, isMessage, isActor, isActorUrl] = await Promise.all([posts.exists(url), messaging.messageExists(url), db.isSortedSetMember('usersRemote:lastCrawled', url),
  // if url is same as id
