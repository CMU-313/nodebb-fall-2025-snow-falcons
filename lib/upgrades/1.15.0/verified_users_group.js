...
module.exports = {
	...
	method: async function () {
		...
		await batch.processSortedSet('users:joindate', async (uids) => {
			...
			await db.sortedSetAdd('group:verified-users:members', verified.map(() => now), verified.map(u => u.uid));
			await db.sortedSetAdd('group:unverified-users:members', unverified.map(() => now), unverified.map(u => u.uid));
		}, {
			batch: 500,
