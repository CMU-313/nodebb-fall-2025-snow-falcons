		...
		batch.processSortedSet('posts:pid', (ids, next) => {
			posts.getPostsFields(ids, ['pid', 'toPid', 'timestamp'], (err, data) => {
				...
				async.eachSeries(data, (postData, next) => {
					...
					async.parallel([async.apply(db.sortedSetAdd, `pid:${postData.toPid}:replies`, postData.timestamp, postData.pid), async.apply(db.incrObjectField, `post:${postData.toPid}`, 'replies')], next);
				}, next);
			});
		}, {
			progress: progress,
