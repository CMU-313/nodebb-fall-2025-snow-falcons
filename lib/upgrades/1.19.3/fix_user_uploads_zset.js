		...
		await batch.processSortedSet('users:joindate', async (uids) => {
			...
			await Promise.all(uids.map(async (uid) => {
				...
				if (uploads.length) {
					...
					await db.sortedSetAdd(key, uploads.map(upload => upload.score), uploads.map(upload => upload.value.slice(1)));
					uploads = await db.getSortedSetMembers(key);
					await db.setObjectBulk(uploads.map(relativePath => [`upload:${md5(relativePath)}`, {
						uid: uid,
					}]));
				}
			}));
