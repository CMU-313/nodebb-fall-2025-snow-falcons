		...
		await batch.processArray(allUids, async (uids) => {
			...
			await Promise.all(userData.map(async (userData, index) => {
				if (!userData || !userData.uid) {
					await db.sortedSetsRemove(['users:joindate', 'users:reputation', 'users:postcount', 'users:flags'], uids[index]);
					if (userData && !userData.uid) {
				...
				totalUserCount += 1;
				await db.sortedSetAddBulk([['users:joindate', userData.joindate || Date.now(), uids[index]], ['users:reputation', userData.reputation || 0, uids[index]], ['users:postcount', userData.postcount || 0, uids[index]]]);
