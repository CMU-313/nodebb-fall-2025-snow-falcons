...
module.exports = {
	...
	method: async function () {
		const defaultPrivileges = ['find', 'read', 'topics:read', 'topics:create', 'topics:reply', 'topics:tag', 'posts:edit', 'posts:history', 'posts:delete', 'posts:upvote', 'posts:downvote', 'topics:delete'];
		const modPrivileges = defaultPrivileges.concat(['posts:view_deleted', 'purge']);
		const globalModPrivs = ['groups:chat', 'groups:upload:post:image', 'groups:upload:post:file', 'groups:signature', 'groups:ban', 'groups:search:content', 'groups:search:users', 'groups:search:tags', 'groups:view:users', 'groups:view:tags', 'groups:view:groups', 'groups:local:login'];
		...
		async function givePrivsToModerators(cid, groupPrefix) {
			...
			const memberPromises = members.map(member => groups.join(privGroups, member));
