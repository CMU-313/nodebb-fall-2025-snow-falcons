...
module.exports = function (Groups) {
	...
	async function updatePrivacy(groupName, isPrivate) {
		...
		const joinPromises = pendingUids.map(uid => Groups.join(groupName, uid));
	...
	async function updateWidgets(oldName, newName) {
		...
		const setAreaPromises = data.areas
			.filter(area => area.data.length)
			.map(area => widgets.setArea(area));
		await Promise.all(setAreaPromises);
	}
	async function updateConfig(oldName, newName) {
		...
		const configPromises = configKeys
			.filter(key => meta.config[key] && meta.config[key].includes(oldName))
			.map(async (key) => {
