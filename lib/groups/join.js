...
module.exports = function (Groups) {
	Groups.join = async function (groupNames, uid) {
		...
		const promises = [db.sortedSetsAdd(groupsToJoin.map(groupName => `group:${groupName}:members`), Date.now(), uid), db.incrObjectField(groupsToJoin.map(groupName => `group:${groupName}`), 'memberCount')];
		if (isAdmin) {
		...
		if (visibleGroups.length) {
			await db.sortedSetAdd('groups:visible:memberCount', visibleGroups.map(groupData => groupData.memberCount), visibleGroups.map(groupData => groupData.name));
		}
	...
	async function createNonExistingGroups(groupsToCreate) {
		...
		// Sequential processing required for group creation
