...
activitypubApi.follow = enabledCheck(async (caller, {
  type,
  id,
  actor
} = {}) => {
...
activitypubApi.unfollow = enabledCheck(async (caller, {
  type,
  id,
  actor
}) => {
...
activitypubApi.create.note = enabledCheck(async (caller, {
  pid,
  post
}) => {
  if (!post) {
    post = (await posts.getPostSummaryByPids([pid], caller.uid, {
      stripTags: false
    })).pop();
    if (!post) {
      return;
  ...
  const {
    activity,
    targets
  } = await activitypub.mocks.activities.create(pid, caller.uid, post);
  await Promise.all([activitypub.send('uid', caller.uid, Array.from(targets), activity), activitypub.feps.announce(pid, activity)]);
});
activitypubApi.create.privateNote = enabledCheck(async (caller, {
  messageObj
}) => {
  const {
    roomId
...
activitypubApi.update.profile = enabledCheck(async (caller, {
  uid
}) => {
  const [object, targets] = await Promise.all([activitypub.mocks.actors.user(uid), db.getSortedSetMembers(`followersRemote:${caller.uid}`)]);
...
activitypubApi.update.category = enabledCheck(async (caller, {
  cid
}) => {
...
activitypubApi.update.note = enabledCheck(async (caller, {
  post
}) => {
  ...
  await Promise.all([activitypub.send('uid', caller.uid, Array.from(targets), payload), activitypub.feps.announce(post.pid, payload)]);
});
activitypubApi.update.privateNote = enabledCheck(async (caller, {
  messageObj
}) => {
...
activitypubApi.delete.note = enabledCheck(async (caller, {
  pid
}) => {
  ...
  const post = (await posts.getPostSummaryByPids([pid], caller.uid, {
    stripTags: false
  })).pop();
  const object = await activitypub.mocks.notes.public(post);
  const {
  ...
  await Promise.all([activitypub.send('uid', caller.uid, Array.from(targets), payload), activitypub.feps.announce(pid, payload)]);
});
activitypubApi.like = {};
activitypubApi.like.note = enabledCheck(async (caller, {
  pid
}) => {
  const payload = {
  ...
  await Promise.all([activitypub.send('uid', caller.uid, [uid], payload), activitypub.feps.announce(pid, payload)]);
});
activitypubApi.announce = {};
activitypubApi.announce.note = enabledCheck(async (caller, {
  tid
}) => {
  const {
    mainPid: pid,
...
activitypubApi.undo = {};
activitypubApi.undo.like = enabledCheck(async (caller, {
  pid
}) => {
  if (!activitypub.helpers.isUri(pid)) {
    return;
...
activitypubApi.flag = enabledCheck(async (caller, flag) => {
  ...
  const reason = flag.reason || flag.reports && flag.reports.filter(report => report.reporter.uid === caller.uid).at(-1).value;
