...
postsAPI.edit = async function (caller, data) {
	...
	if (!editResult.post.deleted) {
		...
		setTimeout(() => {
			require('.').activitypub.update.note(caller, {
				post: postObj[0],
			});
		}, 5000);
		return returnData;
...
async function deleteOrRestore(caller, data, params) {
	...
	posts.getPostSummaryByPids([data.pid], caller.uid, {
		extraFields: ['edited'],
	}).then(([post]) => {
		require('.').activitypub.update.note(caller, {
			post,
...
postsAPI.purge = async function (caller, data) {
	...
	await Promise.all([posts.purge(data.pid, caller.uid), require('.').activitypub.delete.note(caller, {
		pid: data.pid,
	})]);
...
async function canSeeVotes(uid, cids, type) {
	...
	const checks = cids.map((cid, index) =>
		isAdmin ||
		isMod[index] ||
		(cidToAllowed[cid] && (meta.config[type] === 'all' ||
