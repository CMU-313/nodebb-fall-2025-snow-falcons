...
usersAPI.revokeSession = async (caller, {
	...
	for (const sid of sids) {
		// Sequential processing required for session lookup
...
usersAPI.invite = async (caller, {
	...
	for (const email of emailsArr) {
		let invites = 0;
		if (max) {
			// Sequential processing required for invite count check
			// eslint-disable-next-line no-await-in-loop
		...
		// Sequential processing required for email sending
		// eslint-disable-next-line no-await-in-loop
...
usersAPI.search = async function (caller, data) {
	...
	if (!allowed ||
		((data.searchBy === 'ip' || data.searchBy === 'email' ||
		filters.includes('banned') || filters.includes('flagged')) && !isPrivileged)) {
		throw new Error('[[error:no-privileges]]');
	}
