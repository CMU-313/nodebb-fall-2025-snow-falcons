...
Messaging.getPublicRooms = async (callerUid, uid) => {
	...
	const checks = await Promise.all(allRoomData.map(room =>
		room && (!Array.isArray(room.groups) || !room.groups.length || isAdmin ||
...
Messaging.getLatestUndeletedMessage = async (uid, roomId) => {
	...
	let mids;
	while (!done) {
		// Sequential processing required for message retrieval
...
Messaging.hasPrivateChat = async (uid, withUid) => {
	...
	while (index < roomIds.length && !roomId) {
		// Sequential processing required for room checking
		// eslint-disable-next-line no-await-in-loop
...
Messaging.canViewMessage = async (mids, roomId, uid) => {
	...
	const canView = midTimestamps.map(midTimestamp =>
