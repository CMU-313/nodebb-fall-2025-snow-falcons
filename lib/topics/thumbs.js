...
Thumbs.delete = async function (id, relativePaths) {
  ...
  if (toRemove.length && !isDraft) {
    ...
    const mainPid = (await topics.getMainPids([id]))[0];
    await Promise.all([db.incrObjectFieldBy(`topic:${id}`, 'numThumbs', -toRemove.length), Promise.all(toRemove.map(async relativePath => posts.uploads.dissociate(mainPid, relativePath)))]);
  }
  if (toRemove.length) {
    cache.del(set);
  }
