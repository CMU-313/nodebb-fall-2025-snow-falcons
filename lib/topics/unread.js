...
module.exports = function (Topics) {
	...
	async function doesTidHaveUnblockedUnreadPosts(tid, params) {
		...
		let start = 0;
		const count = 3;
		const hasUnblockedUnread = params.topicTimestamp > userLastReadTimestamp;
		if (!params.blockedUids.length) {
		...
		const processBatch = async () => {
			...
			const hasUnblocked = postData.length > 0;
			if (hasUnblocked) {
				return true;
			}
			start += count;
	...
	Topics.markAsRead = async function (tids, uid) {
		...
		const topics = topicScores.filter((t, i) => t.lastposttime &&
			(!userScores[i] || userScores[i] < t.lastposttime || userScores[i] > now));
