...
module.exports = function (Topics) {
  ...
  Topics.post = async function (data) {
    ...
    const [categoryExists, canCreate, canTag, isAdmin] = await Promise.all([parseInt(data.cid, 10) > 0 ? categories.exists(data.cid) : true, privileges.categories.can('topics:create', data.cid, uid), privileges.categories.can('topics:tag', data.cid, uid), privileges.users.isAdministrator(uid)]);
  ...
  async function onNewPost({
    ...
    const [[postData], [userInfo]] = await Promise.all([posts.getPostSummaryByPids([pid], uid, {}), posts.getUserInfoForPosts([postOwner], uid)]);
