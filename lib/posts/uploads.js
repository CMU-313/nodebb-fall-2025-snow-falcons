...
module.exports = function (Posts) {
	...
	Posts.uploads.dissociate = async function (pid, filePaths) {
		...
		if (!meta.config.preserveOrphanedUploads) {
			const deletePaths = (await Promise.all(filePaths.map(async filePath =>
				(await Posts.uploads.isOrphan(filePath)) ? filePath : false))).filter(Boolean);
			...
			await Promise.all(uploaderUids.map((uid, idx) =>
				uid && isFinite(uid) ? user.deleteUpload(uid, uid, deletePaths[idx]) : null).filter(Boolean));
