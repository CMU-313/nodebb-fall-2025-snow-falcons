...
async function fireFilterHook(hook, hookList, params) {
	...
	// Sequential processing required for filter hooks
	for (let i = 0; i < hookList.length; i++) {
		// eslint-disable-next-line no-await-in-loop
		paramsResult = await fireMethod(hookList[i], paramsResult);
	}
	return paramsResult;
}
async function fireActionHook(hook, hookList, params) {
	...
	const actionPromises = hookList.map(async (hookObj) => {
		if (typeof hookObj.method !== 'function') {
			if (global.env === 'development') {
				winston.warn(`[plugins] Expected method for hook '${hook}' in plugin '${hookObj.id}' not found, skipping.`);
			}
...
async function fireStaticHook(hook, hookList, params) {
	...
	const staticPromises = hookList.map(async (hookObj) => {
		try {
...
async function fireResponseHook(hook, hookList, params) {
	...
	const responsePromises = hookList.map(async (hookObj) => {
		if (typeof hookObj.method !== 'function') {
			if (global.env === 'development') {
				winston.warn(`[plugins] Expected method for hook '${hook}' in plugin '${hookObj.id}' not found, skipping.`);
			}
