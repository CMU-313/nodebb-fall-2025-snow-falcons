...
Data.getPluginPaths = async function () {
	...
	const pluginPaths = plugins.filter(plugin => plugin && typeof plugin === 'string').map(plugin => path.join(paths.nodeModules, plugin));
...
Data.loadPluginInfo = async function (pluginPath) {
	const [packageJson, pluginJson] = await Promise.all([fs.promises.readFile(path.join(pluginPath, 'package.json'), 'utf8'), fs.promises.readFile(path.join(pluginPath, 'plugin.json'), 'utf8')]);
	let pluginData;
	let packageData;
	try {
...
Data.getStaticDirectories = async function (pluginData) {
	...
	async function processDir(route) {
		if (!validMappedPath.test(route)) {
			winston.warn(`[plugins/${pluginData.id}] Invalid mapped path specified: ${route}. Path must adhere to: ${validMappedPath.toString()}`);
			return;
		}
		const dirPath = await resolveModulePath(pluginData.path, pluginData.staticDirs[route]);
		if (!dirPath) {
			winston.warn(`[plugins/${pluginData.id}] Invalid mapped path specified: ${route} => ${pluginData.staticDirs[route]}`);
			return;
		}
		try {
...
Data.getScripts = async function getScripts(pluginData, target) {
	...
	const scripts = [];
	const scriptPromises = input.map(async (filePath) => {
