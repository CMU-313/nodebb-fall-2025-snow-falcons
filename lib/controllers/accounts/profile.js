...
async function getPosts(callerUid, userData, setSuffix) {
	...
	do {
		// Sequential processing required for post pagination
		...
		if (pids.length) {
			({
				pids,
				// Sequential processing required for post filtering
				// eslint-disable-next-line no-await-in-loop
			...
			// Sequential processing required for post retrieval
			// eslint-disable-next-line no-await-in-loop
			const p = await posts.getPostSummaryByPids(pids, callerUid, {
				stripTags: false,
			});
			postData.push(...p.filter(p =>
				p && p.topic &&
				(isAdmin || isModOfCid[p.topic.cid] ||
