...
topicsController.get = async function getTopic(req, res, next) {
	...
	if ((req.params.post_index && !utils.isNumber(req.params.post_index) && req.params.post_index !== 'unread') ||
	...
	const invalidPagination = settings.usePagination &&
...
async function addOGImageTags(res, topicData, postAtIndex) {
	...
	if (topicData.thumbs) {
		...
		const thumbs = topicData.thumbs.filter(t =>
			t && images.every(img => path.normalize(img.name) !== path.normalize(url + t.url)));
