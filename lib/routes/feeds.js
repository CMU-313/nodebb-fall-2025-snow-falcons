...
async function generateForTopic(req, res, next) {
	...
	if (await validateTokenIfRequiresLogin(!userPrivileges['topics:read'], topic.cid, req, res)) {
		...
		replies.forEach((postData) => {
			if (!postData.deleted) {
				const dateStamp = new Date(parseInt(
					parseInt(postData.edited, 10) === 0 ? postData.timestamp : postData.edited,
					10,
				)).toUTCString();
				feed.item({
...
async function generateTopicsFeed(feedOptions, feedTopics, timestampField) {
	...
	const feedItemPromises = feedTopics.map(topicData => addFeedItem(topicData));
	await Promise.all(feedItemPromises);
	return feed;
}
